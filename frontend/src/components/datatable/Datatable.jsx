...
const Datatable = ({ columns }) => {
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction">
            <div
              className="bg-purple-500 hover:bg-purple-700 text-white font-bold py-1 px-4 rounded cursor-pointer"
  ...
  const filteredList = useMemo(() => {
    ...
    return list.filter((item) => {
      ...
      const searchableString = `${item.category}${item.type} ${item.email} ${item.mobile} ${item.country} ${item.vehicleType}  `;
